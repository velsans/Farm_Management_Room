...
actual fun platformExport(snapshot: FarmSnapshot): ByteArray {
...
actual fun platformImport(bytes: ByteArray): FarmSnapshot {
...
private fun Workbook.writeCrops(crops: List<CropEntity>) {
    ...
    crops.forEachIndexed { index, crop ->
        sheet.createRow(index + 1).write(
            crop.id, crop.name, crop.variety, crop.fieldName, crop.area, crop.season, crop.sowingDate, crop.notes,
        )
    }
}
...
private fun Workbook.writeExpenses(expenses: List<ExpenseEntity>) {
    ...
    expenses.forEachIndexed { index, expense ->
        sheet.createRow(index + 1).write(
            expense.id, expense.cropId, expense.category, expense.applicationRound ?: "", expense.amount, expense.expenseDate, expense.notes,
        )
    }
}
...
private fun Workbook.writeHarvests(harvests: List<HarvestEntity>) {
    ...
    harvests.forEachIndexed { index, harvest ->
        sheet.createRow(index + 1).write(
            harvest.id, harvest.cropId, harvest.harvestDate, harvest.quantityKg, harvest.managementNotes,
        )
    }
}
...
private fun Workbook.writeSales(sales: List<SaleEntity>) {
    ...
    sales.forEachIndexed { index, sale ->
        sheet.createRow(index + 1).write(
            sale.id, sale.cropId, sale.saleDate, sale.quantityKg, sale.pricePerKg, sale.totalIncome, sale.buyerName, sale.buyerPhone, sale.notes,
        )
    }
}
...
private fun Workbook.readCrops(): List<CropEntity> =
    getSheet("Crops")?.dropHeader()?.map { row ->
        CropEntity(
            id = row.longAt(0), name = row.stringAt(1), variety = row.stringAt(2), fieldName = row.stringAt(3),
            area = row.doubleAt(4), season = row.stringAt(5), sowingDate = row.stringAt(6), notes = row.stringAt(7),
        )
    }.orEmpty()
...
private fun Workbook.readExpenses(): List<ExpenseEntity> =
    getSheet("Expenses")?.dropHeader()?.map { row ->
        ExpenseEntity(
            id = row.longAt(0), cropId = row.longAt(1), category = row.stringAt(2), applicationRound = row.optionalIntAt(3),
            amount = row.doubleAt(4), expenseDate = row.stringAt(5), notes = row.stringAt(6),
        )
    }.orEmpty()
...
private fun Workbook.readHarvests(): List<HarvestEntity> =
    getSheet("Harvests")?.dropHeader()?.map { row ->
        HarvestEntity(
            id = row.longAt(0), cropId = row.longAt(1), harvestDate = row.stringAt(2), quantityKg = row.doubleAt(3), managementNotes = row.stringAt(4),
        )
    }.orEmpty()
...
private fun Workbook.readSales(): List<SaleEntity> =
    getSheet("Sales")?.dropHeader()?.map { row ->
        SaleEntity(
            id = row.longAt(0), cropId = row.longAt(1), saleDate = row.stringAt(2), quantityKg = row.doubleAt(3), pricePerKg = row.doubleAt(4),
            buyerName = row.stringAt(6), buyerPhone = row.stringAt(7), notes = row.stringAt(8),
        )
    }.orEmpty()
...
private fun Row.optionalIntAt(index: Int): Int? = getCell(index)?.displayValue()?.takeIf { it.isNotBlank() }?.toDoubleOrNull()?.toInt()
...
private fun Cell.displayValue(): String = when (cellType) {
    CellType.NUMERIC -> {
